...
const create = (baseURL = config.API_URL) => {
  ...
  const removeDeviceToken = token =>
    api.delete('/pushTokens', {device_token: token})
  ...
  const getSuggestedUsers = (id, query, search) =>
    api.get(`/users/suggested`, {query, search})
  ...
  const getSubComments = (id, query) =>
    api.get(`/comments/${id}/comments`, {query})
  const addSubComment = (id, text) =>
    api.post(`/comments/${id}/comments`, {text})
  ...
  const getConversation = (type, target) =>
    api.get(`/conversations/${type}/${target}`)
  const getConversations = (id, query) =>
    api.get(`/users/${id}/conversations`, {query})
  const openConversation = (type, target) =>
    api.post(`/conversations`, {type, target})
  const readConversation = id => api.post(`/conversations/${id}/read`)
  const sendMessage = message => api.post(`/messages`, message)
  const getMessages = (id, query) =>
    api.get(`/conversations/${id}/messages`, {query})
