...
class BandCreateScreen extends Component {
  ...
  async onSubmit () {
    ...
    let response = await api.addBand({
      name,
      genres,
      province,
      district,
    ...
    return (
      <KeyboardAwareScrollView style={styles.mainContainer}>
          ...
          <TouchableOpacity
            ...
            {this.state.genres.length ? (
              <Text style={styles.value}>
                {this.state.genres.map(item => `#${item}`).join(' ')}
              </Text>
            ) : (
        ...
        <View style={styles.instrumentsGroup}>
          <Text style={styles.groupLabel}>
            {I18nJs.t('instruments').toUpperCase()}
          </Text>
          <View style={styles.row}>
            <View style={styles.instruments}>
              {this.state.selectedInstruments &&
              this.state.selectedInstruments.length ? (
                this.state.selectedInstruments.map((selected, index) => (
                  <TouchableOpacity
                    key={selected}
                    onPress={() => this.onRemoveInstrument(index)}>
                    <View style={styles.tag}>
                      ...
                      <Image
                        source={{
                          uri: url(
                            _.find(this.props.instruments, {_id: selected})
                              .image,
                          ),
                        }}
                ...
                <Text style={styles.selectInstruments}>
                  {I18nJs.t('selectInstruments')}
                </Text>
              )}
            </View>
            <TouchableOpacity
              style={{padding: 5}}
              onPress={() => this.onSelectInstrumentsPress()}>
        ...
        <ModalFilterPicker
          ...
          onSelect={value =>
            this.setState({district: value, showDistrictPicker: false})
          }
