...
class ChatScreen extends Component {
  ...
  async componentWillMount () {
    ...
    if (user) {
      Actions.refresh({
        ...
        right: () => (
          <TouchableOpacity style={{paddingHorizontal: 10}}>
            <Avatar source={user.avatar} size={30} />
          </TouchableOpacity>
        ),
      })
    } else if (band) {
      Actions.refresh({
        ...
        right: () => (
          <TouchableOpacity style={{paddingHorizontal: 10}}>
            <SquareImage source={band.cover} size={30} />
          </TouchableOpacity>
        ),
      })
    }
  ...
  componentWillReceiveProps (nextProps) {
    if (
      ...
      nextProps.recieveMessage.asMutable(true).conversation_id ===
      ...
      const lastMessage = moment
        .utc(_.min(_.map(messages, 'created_at')))
        .toDate()
      this.setState({
        messages,
        lastMessage,
  ...
  renderSend (props) {
    return (
      <Send {...props} containerStyle={{height: 'auto'}}>
