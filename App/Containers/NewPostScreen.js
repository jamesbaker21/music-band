...
class NewPostScreen extends Component {
  ...
  async componentWillMount () {
    Actions.refresh({
      // rightImage: Images.flyPaper,
      rightTitle: 'Save',
      onRight: this.onSubmit.bind(this),
      // headerRight: this.rightButton.bind(this),
    })
    ...
    const fields = [
      {
        name: 'song',
        filename:
          song.filename || /video\//.test(song.mime)
            ? 'video.mp4'
            : 'audio.mp3',
        type: song.mime,
    ...
    try {
      ...
      this.setState({songs: [...this.state.songs, video]}, () => {
        this.dialogInput.show({
          text: '',
          onSubmit: name =>
            this.onEnterSongName(name, this.state.songs.length - 1),
        })
      })
    } catch (_) {}
  }
      ...
      <View>
        {!item.isPlaying ? (
          <View style={styles.videoPreview}>
            <Image style={styles.videoCover} source={{uri: item.image.path}} />
              ...
              <View style={{flex: 1}}>
                <TouchableOpacity
                  onPress={() =>
                    this.dialogInput.show({
                      text: item.name,
                      onSubmit: name => this.onEnterSongName(name, index),
                    })
                  }>
                  <Text style={styles.videoTitle}>
                    {item.name || I18nJs.t('enterSongName')}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() =>
        ...
        <TouchableOpacity
          ...
          onPress={() => this.onStopVideo(index)}>
  ...
  renderAlbum ({item, index}) {
    ...
    return (
      <TouchableOpacity
        ...
        {this.state.albumId === item._id && (
          <Image source={Images.checked} style={styles.checkerAlbum} />
        )}
      </TouchableOpacity>
    )
  }
            ...
            <View>
                ...
                <View style={styles.albumsView}>
                  <View style={styles.albums}>
                    <TouchableOpacity
                      style={styles.addAlbumButton}
                      onPress={() =>
                        Actions.newAlbumScreen({
                          getAlbums: this.getAlbums.bind(this),
                        })
                      }>
                      <Image
                        source={Images.iconPlusRed}
                        style={styles.addButtonImage}
                      />
                      <Text style={styles.addAlbumText}>
                        {I18nJs.t('createAlbum')}
                      </Text>
                    </TouchableOpacity>
                    <FlatList
                      data={albums}
              ...
              <View style={styles.bottomBar}>
                ...
                <TouchableOpacity onPress={() => this.ActionSheet.show()}>
                  <Image
                    source={Images.imageButton}
                    style={styles.buttonImage}
                  />
                </TouchableOpacity>
                <TouchableOpacity>
                  <Image
                    source={Images.locationButton}
                    style={styles.buttonImage}
                  />
                </TouchableOpacity>
              </View>
