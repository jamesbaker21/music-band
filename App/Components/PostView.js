...
class PostView extends Component {
  ...
  render () {
    ...
    return (
      <View style={styles.article}>
        <TouchableOpacity
          onPress={() =>
            this.props.showProfileOnPress && Actions.profileScreen({user})
          }
        ...
        {!!item.images && (
          <PhotoGrid
            source={_.map(_.map(item.images, 'file'), url)}
            onPressImage={(event, source) =>
              this.showImage(source, item.images)
            }
          />
        )}
    ...
    return (
      <View style={styles.postStatus}>
        <Image
          ...
          style={[styles.iconTiny, {marginRight: 5}]}
        />
        <Text style={[styles.lastUpdateText, {flex: 1}]}>
          {item.view_count || 0} {I18nJs.t('views')}
        </Text>
        <TouchableOpacity
          style={styles.buttonSmall}
          onPress={() => Actions.postCommentScreen({post: item})}>
        ...
        <TouchableOpacity
          style={styles.buttonSmall}
          onPress={() => this.onLike(item, !item.is_liked)}>
          {/* <RNShineButton
            shape={'heart'}
  ...
  showImage (source, images) {
    const imageIndex = _.findIndex(
      images,
      image => url(image.filePath) === source,
    )
    ...
    return (
      <View key={song._id}>
        <TouchableOpacity
          ...
          onPress={() => Actions.videoScreen({song, post})}>
          <ImageLoad
          ...
          <View style={styles.videoContent}>
            <View style={{flex: 1}}>
              ...
              <Text style={styles.instrumentTags}>
                {song.genres && song.genres.map(genre => `#${genre}`).join(' ')}
              </Text>
            </View>
  ...
  renderAudio (song, post) {
    return (
      <View key={song._id}>
        <TouchableOpacity
          style={styles.row}
          onPress={() => Actions.playerScreen({song, post})}>
          <SquareImage
            source={song.album ? song.album.cover : null}
            size={40}
          />
          <View style={styles.songContent}>
            <Text style={styles.bandName}>{song.name}</Text>
            <Text style={styles.lastUpdateText}>
              {song.album ? song.album.name : I18nJs.t('noAlbum')}
            </Text>
          </View>
